...
/* Two shift operations (all bigints are signed, so there is no such
   thing as an unsigned shift right) */
bigint bigint_shift_left(bigint l, int shamt);
bigint bigint_shift_right(bigint l, int shamt);

/* There are seven comparison operations.  They return 1 for true, or
   0 for false. */
